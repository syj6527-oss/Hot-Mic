...
const MODE_GROUPS = {
    ...
    sports: [
        { key:'ball', label:'구기', subs: [
            { style: `【축구 중계 st】 4대 구기 중 **가장 긴 빌드업** — 대부분 차분한 상황 설명("후반 43분, 스코어 1대1")으로 흐르다 공이 골문 앞에 닿을 때만 한 줄씩 끊어 폭발. 끊어치기는 매번 다른 골 앞 상황으로 변주(문전 혼전·헤더·발리·중거리·코너킥 등에서 골라). 골 순간 딱 한 번 모음 늘려 터뜨리고("들어갔습니다아아아!!!"), 직후 차분히 의미 부여. 용어: 오버래핑·크로스·문전 혼전·VAR·추가시간·극장골·오프사이드. 해설위원 한 줄(선택).`, labels: ["중계","실황","현장"] },
            { style: `【야구 중계 st】 4대 구기 중 **가장 여백이 많은 종목** — 긴 정적과 뜸이 핵심. "9회말 2아웃. 세트 포지션 들어갑니다. 사인이 길어집니다. 숨소리조차 들릴 만큼 조용하네요." 한 박자 느린 여유 + 통계·기록 인용("이게 4년차의 여유죠"). 그러다 한 방으로 폭발(타구 따라 한 줄씩, 매번 다른 타구로 변주 — 좌익수 앞·담장 넘어·병살·끝내기 등). 정적 뒤의 폭발이 포인트. 용어: 세트포지션·풀카운트·병살·끝내기·방어율·타율.`, labels: ["중계","해설","9회말"] },
            { style: `【농구 중계 st】 4대 구기 중 **가장 빠르고 끊김 없는 종목** — 공수 전환이 쉴 새 없어 호흡이 거의 안 끊긴다. 속공·돌파·3점 라인에서 숨이 가빠진다("공 잡고— 돌파! 빠집니다! 3점 라인— 던졌— 들어가!!!" 식, 매번 다른 공격으로 변주). 24초·픽앤롤·앨리웁·덩크·리바운드·자유투·버저비터. 멈추지 않는 속도감이 축구·야구와의 차이.`, labels: ["중계","코트","속공"] },
            { style: `【미식축구 중계 st】 NFL식 — **매 플레이가 완전히 끊겼다 리셋되는** 호흡이 핵심(작전 → 스냅 → 폭발 → 정렬, 다시). "3rd & 7. 스냅— 백 스텝— 던집니다! 사이드라인! 잡았어요, 퍼스트 다운!"(매번 다른 플레이로 변주 — 러싱·색·인터셉트·필드골 등). 다운·야드·쿼터백·터치다운·턴오버. 끊고-모이고-터지는 단속적 리듬이 다른 구기와 다르다.`, labels: ["중계","다운","터치다운"] },
        ] },
    ...
    wiki: [
        ...
        { key:'encyclopedia', label:'백과사전', subs: [
            { style: `【백과사전 st】 정식 백과사전 표제어체(국립생물자원관·학술 백과 느낌). 구성 옵션(장면에 맞게 일부만):
① 표제어 헤더 — 한글명 + 영문/한자 학명 + **이명 나열**("금등화, 타태화, 대화능소…"처럼 가짜 이명 2~4개).
② 분류표 — "분류 / 서식지 / 학명 / 분포" 같은 라벨:값. 분류는 계층 경로로("현화식물문 > 목련강 > … "처럼 그럴듯하게), 학명은 이탤릭 느낌의 가짜 라틴어.
③ 섹션 서술(개요·형태·생태·이용·해설 중) — **측정치를 박은 건조한 기술**("길이 8-10m, 작은잎 5-9장, 길이 3-6cm", "꽃은 7-8월에 피며 9-10월에 익는다", "관상용으로 식재하며 ~로 쓴다").
④ 해설 = **비교 구분**("○○에 비해 ~이며, ~이므로 구분된다").
사소하고 시시한 사실을 거창한 정의문·측정치에 천연덕스럽게 끼워 웃긴다. 감정 배제, 표제어의 무심함 그 자체.`, labels: ["표제어","정의","백과사전"] },
        ] },
        ...
        { key:'fielddex', label:'도감', subs: [
            { style: `【도감 st】 생물도감 항목체. 대상을 분류 항목표(라벨: 값)로 정리한다. 라벨 풀(골라서): "학명 / 이명 / 분류 / 서식지 / 먹이 / 천적 / 습성 / 울음소리 / 활동 시간 / 개체수". 예: "학명: Homo cubiculus / 이명: 집순이, 방돌이 / 분류: 대형 영장류 / 서식지: 원룸 / 먹이: 김치볶음밥 / 천적: 배달 앱 종료 / 습성: 먹이를 섭취하면서도 주변 개체를 지속 관찰한다." 학명은 그럴듯한 가짜 라틴어, 이명은 별명 2~3개. 객관적 분류 양식을 빌려 시시하고 웃긴 진실을 항목 사이에 툭툭 박는다(다큐가 '장면을 서술'한다면 도감은 '분류표로 놀린다'). 건조한 도감 톤, 항목은 라벨: 값 형식.`, labels: ["도감","분류","생태"] },
        ] },
    ],
    news: [
