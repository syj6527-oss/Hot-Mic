...
const MODE_GROUPS = {
    ...
    sports: [
        { key:'ball', label:'구기', subs: [
            { style: `【축구 중계 st】 4대 구기 중 **가장 긴 빌드업** — 넓은 그라운드를 **거시적으로 조망**하며 후방 빌드업·하프스페이스 공간 창출을 흐름으로 짚다가, 공이 골문 앞에 닿을 때만 한 줄씩 끊어 폭발. 끊어치기는 매번 다른 골 앞 상황으로 변주(문전 혼전·헤더·발리·중거리·코너킥 등에서 골라). 골 순간 딱 한 번 모음 늘려 터뜨리고("들어갔습니다아아아!!!"), 직후 차분히 의미 부여. 용어: 후방 빌드업·하프스페이스·오버래핑·크로스·문전 혼전·VAR·추가시간·극장골·오프사이드. 해설위원 한 줄(선택).`, labels: ["중계","실황","현장"] },
            { style: `【야구 중계 st】 4대 구기 중 **가장 여백이 많은 종목** — 긴 정적과 뜸이 핵심. "세트 포지션 들어갑니다. 사인이 길어집니다. 숨소리조차 들릴 만큼 조용하네요." 한 박자 느린 여유 + **인라인 스탯**(타율·볼카운트·방어율 — "최근 10경기 타율 0.3할대", "볼카운트 2-1"). 그러다 한 방으로 폭발(타구 따라 한 줄씩, 매번 다른 타구로 변주 — 좌중간·담장 넘어·병살·끝내기 등). **색깔 표현**(직유 한 방: "화살처럼", "로켓처럼"). 정적 뒤의 폭발이 포인트.
[구조 변주 씨앗 — 매번 다른 표현으로, 복붙 금지] 오프닝(아나운서 인사 + 양 팀 시즌 폼 프리뷰), 선발 투수 소개(구종·최근 피칭), 후반 긴장(회차·점수차·"긴장감이 감도는"), 관중 반응("응원 소리가 가득"), 종료 + 최종 스코어 + **MVP 선정**. 장면 흐름에 맞게 일부만 골라 쓰되 문장은 매번 새로.
용어: 세트포지션·풀카운트·병살·끝내기·방어율·타율. ※"타율 0.350/좌중간/담장 넘어"식 고정 문구 반복 금지 — 리듬만 빌리고 표현은 변주.`, labels: ["중계","해설","9회말"] },
            { style: `【농구 중계 st】 4대 구기 중 **가장 빠르고 끊김 없는 종목** — 공수 전환이 쉴 새 없어 호흡이 거의 안 끊긴다. **짧은 외침을 연발**로 쏟아낸다("돌파 성공! 아~ 좋아요!", "또 들어갑니다!", "첫 번째 시도, 들어가나요!"). 슛 성공 순간 폭발("던집니다!!!! 와아아아아!!"), **거리·높이 감탄 드립**(3점·롱샷에 "이게 몇 미텁니까!!!"), 연속 실패도 한 박자 안 쉬고 빠르게("아! 또 안 들어갔고!"). 코트 위 세부 전술(픽앤롤·아이솔레이션)을 숨 가쁘게 짚되, 흥분이 우선. 매번 다른 공격으로 변주. 24초·앨리웁·덩크·리바운드·자유투·버저비터. 멈추지 않는 속도감이 축구·야구와의 차이. ※"이게 몇 미텁니까"식 특정 펀치라인 복붙 금지 — 무호흡 외침 연발·거리 감탄이라는 *리듬*만 빌릴 것. 격투·다른 종목 용어 섞지 말 것.`, labels: ["중계","코트","속공"] },
            { style: `【미식축구 중계 st】 NFL식 — **매 플레이가 완전히 끊겼다 리셋되는** 호흡이 핵심(작전 → 스냅 → 폭발 → 정렬, 다시). "3rd & 7. 스냅— 백 스텝— 던집니다! 사이드라인! 잡았어요, 퍼스트 다운!"(매번 다른 플레이로 변주 — 러싱·색·인터셉트·필드골 등). **페이크 텐션**도 좋다: 끊어치기+모음 늘리기로 가짜 위기 연출했다가 반전("핸드오프 하려다— 아차차 넘어집니다아아— 만 간신히 중심 잡고 딥패스! 터치다운!"). **착지 장치 = 리플레이 판독**("굉장히 좋은 플레이…인 줄 알았는데! 돌려보니까 그냥 헐리우드 액션이었습니다") — 거창하게 띄우고 슬로모션으로 시시하게 까발리는 게 미식축구 고유 개그. 다운·야드·쿼터백·러닝백·핸드오프·터치다운·턴오버. ※"헐리우드 액션"식 특정 펀치라인 복붙 금지 — '가짜 위기→반전', '돌려보니까 ~였다'라는 *구조*만 변주 씨앗으로.`, labels: ["중계","다운","터치다운"] },
        ] },
        { key:'fight', label:'격투', subs: [
            { style: `【격투기 중계 st】 복싱·킥복싱 등 **타격 위주** — 그래플링보다 주먹·발의 교환이 중심. 거리 재는 동안 담담히("거리를 좁힙니다, 반응 살핍니다") → 타격이 꽂히면 한 줄씩 빠르게(잽·훅·카운터·콤비네이션, 매번 다른 타격 조합으로 변주) → 다운/피니시 순간 폭발("경기 종료!!! 엄청난 피니시!"). 가드·클린치·다운 카운트·라운드 용어.`, labels: ["중계","라운드","경기"] },
            { style: `【UFC·옥타곤 st】 종합격투 — **케이지 안 거리싸움·테이크다운·그래플링 비중이 큰 게** 타격 위주 격투기와의 차이. "케이지 등지고— 테이크다운 시도! 막혔어요! 클린치, 무릎! 그라운드로 끌고 갑니다!"(매번 다른 전개로 변주). 체급·라운드·서브미션·탭·파운딩·옥타곤 컨트롤. 타격만큼 그래플링·포지션 싸움을 비춘다.`, labels: ["옥타곤","라운드","서브미션"] },
        ] },
        { key:'esports', label:'e스포츠', subs: [
            { style: `【e스포츠 중계 st】 롤·스타 중계. 라인전·운영 구간은 의외로 차분하게 상황을 정리합니다("CS 격차 30개, 바텀 라인 밀어두고 시야부터 잡습니다"). 한타·교전이 열리는 순간부터 빠르게 몰아침("각 봤어요! 이니시 들어갑니다! 점멸 빠졌고요! 이거 잘렸는데요?!" 식, 교전마다 다른 전개로 변주). 한타 끝나면 "GG"로 정리. 용어는 폭격하되 교전 전까진 흥분을 아껴두세요.`, labels: ["중계","LIVE","한타"] },
        ] },
    ...
    wiki: [
        { key:'wikipedia', label:'위키', subs: [
            { style: `【위키백과 st】 온라인 위키 백과 문서. 신호: 맨 위 "위키백과, 우리 모두의 백과사전" 부제 + **정의문 도입**("○○(한자/학명: 가짜 라틴어 이탤릭 느낌)은(는) ~에 속하는 ~이다"). **우측 정보상자 느낌**(대상에 맞는 라벨:값 — 생물이면 역/계/문/강/목/과/속/종 계층 + 학명 + 향명("영어: …, 일본어: …" 각국어 나열), 인물·사물이면 그에 맞는 항목). 본문은 중립·객관 "~이다/~로 여겨진다/논란이 있다/일각에서는". 측정치 박기("신장 19-23cm, 체중 58-101g"). 각주는 위키답게 — **1부터 순서대로 정직하게 달지 말 것**: [3][8][14]처럼 띄엄띄엄+큰 숫자 섞고 일부 문장만, 맨 아래 참고주는 학술 인용 흉내(저자·연도·저널·doi). 가끔 학술논문체로 변주. ※백과사전 결과의 차이 = **'우리 모두의 백과사전' 부제 + 우측 정보상자 + 정의문이 위키 시그니처**(백과사전은 [요약]박스+목차+섹션). ※사관백과(나무위키)와의 차이 = 철저히 중립·격식 — **취소선·'여담으로'·'(아니라고는 안 했다)'·깐죽 드립 금지**.`, labels: ["개요","항목","편집"] },
            { style: `【나무위키(사관백과) st】 덕질 위키체. 신호: **소수점 다단계 헤더**("2. 평가 / 4.1. 감독 스타일 / 4.1.1.1. 부임 전")에 [편집] 링크 느낌. **평어체 "~다/~한다" 종결**("자유도가 없다는 평을 듣고 있다", "~하는 것이다"). 진지하게 분석하다 깐죽대는 팩트 폭발("정해진 길 외에는 지뢰밭이다", "이건 뭐 ~도 아니고 하는 말이 절로 나올", "말 그대로 눈물겹다"). **독자 호출**("여기까지 읽었으면 알겠지만 ~"), **선긋기**("~하는 셈이다", "~란 뜻은 아니다"), **굵게 강조** 빈발("하지만 ~인 게 문제가 아니라 ~인 게 문제다"). 각주 [13][14][15] 불규칙+본문 중간중간. 취소선 드립(~~사실 그냥 곁에 두고 싶은 거~~), "(아니라고는 안 했다)", "여담으로". 시시콜콜 나열하다 한 줄 촌철살인. ※위키백과(중립·격식)와의 차이 = **드립·주관·깐죽** — 소수점 헤더·취소선·독자 호출·굵게강조 중 **최소 한두 개 필수**. 무미건조하면 위키백과랑 안 갈린다.`, labels: ["여담","나무위키","ㅇㅇ"] },
        ] },
        ...
        { key:'encyclopedia', label:'백과사전', subs: [
            { style: `【백과사전 st】 정식 백과사전 항목(다음·학술 백과 느낌). 생물뿐 아니라 **인물·행사·개념 등 무엇이든** 거창한 백과 항목으로 박제. 구성 옵션(장면에 맞게 일부만):
① 표제어 헤더 — 한글명 + **한자/영문 병기**("이이 李珥", "사회적 거리두기, social distancing, 社會的 距離") + 이명·별칭 나열(2~4개).
② 정보표(라벨:값) — **대상마다 라벨이 다름**: 인물=출생/사망/국적/본관/자·호, 행사=개최지/기간/규모/주관처/종목, 생물=분류/서식지/학명/분포.
③ [요약] 박스 — 핵심을 한 문단으로 압축(맨 앞에 "[요약]" 표시).
④ 목차 — 개요·생애·사상… 또는 정의·배경·전개… (대상별로 다르게).
⑤ 섹션 서술 — 연도+사건 나열("1564년 ~하였다"), 측정치·수치 박기, **한자 병기 빈번**(性理學·時弊처럼). 중립·건조.
시시한 RP 한 장면을 [요약]·목차·섹션까지 갖춘 정식 항목으로 박제하는 괴리가 개그. 감정 배제, 표제어의 무심함. ※위키백과 결과의 차이 = **[요약] 박스 + 목차 + 섹션 prose가 백과사전 시그니처**(위키백과는 '우리 모두의 백과사전' 부제 + 우측 정보상자). ※도감과의 차이 = 표가 아니라 **문단 서술 중심**, '천적·먹이·울음소리' 같은 드립 라벨 금지(도감 몫).`, labels: ["표제어","정의","백과사전"] },
        ] },
        { key:'chronicle', label:'연표·연감', subs: [
            { style: `【연표·연감 st】 시간순 또는 통계 기록만으로 서술. 매번 둘 중 하나로(혼용 말 것):
(가) 연표(Timeline) — 사건을 시간순 한 줄씩, 시각/연도를 앞세워 나열. 변주: ⑴ 시각별("09:24 기상 / 09:25 보행 이상 발견 / 09:26 이부프로펜 권고"), ⑵ 연도별 빽빽이("기원전 2333 ○○ 건국 / 1969 ○○ 구축 / 1989 ○○ 발명"), ⑶ 점선 타임라인체(항목명 → 연도(이탤릭) → 한 줄 설명), ⑷ 2단 병렬(좌:한쪽 연표 / 우:다른 쪽 연표 나란히, 예 "한국사 | 세계사"). 해석·감정·수식 없이 "~하다/~되다" 건조 종결.
(나) 연감(Yearbook) — **특정 연도 데이터를 카테고리별 표로**. 열 구성: 분류 / 세부 항목 / 결산 수치 / **전년 대비 증감(+4.2%·-1.5%)**. 예: "시장 규모 | 총매출 | 25조 3,400억 | +4.2%". 통계·수치·증감 부호 중심, "올해의 키워드" 같은 요약 행 가능.
담백할수록 웃기다. 마지막 한 항목/한 칸에만 시시한 진실이 슬쩍. ※연표=시간순 한 줄 나열 / 연감=카테고리×수치 표 — 둘을 섞지 말 것.`, labels: ["연표","연감","기록"] },
        ] },
        { key:'fielddex', label:'도감', subs: [
            { style: `【도감 st】 생물도감 항목체. 대상을 분류 항목표(라벨: 값)로 정리한다. 라벨 풀(골라서): "학명 / 이명 / 분류학적위치(목>과>속 계층 경로) / 분류 / 서식지 / 먹이 / 천적 / 습성 / 울음소리 / 활동 시간 / 개체수". 예: "학명: Homo cubiculus / 이명: 집순이, 방돌이 / 분류: 대형 영장류 / 서식지: 원룸 / 먹이: 김치볶음밥 / 천적: 배달 앱 종료 / 습성: 먹이를 섭취하면서도 주변 개체를 지속 관찰한다." 학명은 그럴듯한 가짜 라틴어, 이명은 별명 2~3개. 객관적 분류 양식을 빌려 시시하고 웃긴 진실을 항목 사이에 툭툭 박는다(다큐가 '장면을 서술'한다면 도감은 '분류표로 놀린다'). 건조한 도감 톤, 항목은 라벨: 값 형식. ※백과사전과의 차이 = **전부 라벨:값 표, 문단 서술 금지**. 측정치 박은 학술 문단(백과사전식 "길이 8-10m…")으로 새지 말 것. **항목 값 자체가 펀치라인**(천적·먹이·습성이 농담).`, labels: ["도감","분류","생태"] },
        ] },
    ],
    news: [
...
[댓글 말투 옵션 풀 — 골라서 섞기] 타임스탬프 인용("0:45 이 부분 ㅋㅋㅋ"), ㅋ 한 줄 도배("ㅋㅋㅋㅋㅋㅋㅋ"), 대구 드립(같은 형식 두 줄로 비틀기 "2022년 ~ / 2026년 ~"), 절별 분석체("1절 ~ 잡고 / 2절 ~ 증폭 / 3절 ~ 터뜨림"), 영어 베댓 + "한국어로 번역" 줄, "알고리즘이 나를 여기로", "이걸 왜 이제 봤지", "이 게임만 올리면 조회수 잘나오는 우주하마면 개추". 정보·리뷰 채널이면 진지 분석 + 가격 분해 드립("○○값 6천원 테크닉값 4만4천원"). 드립성 닉네임(@한글닉+영숫자꼬리, 예 @멍구티비·@user-yc5vs8nv3n).
※영어 댓글 섞기: 베스트 댓글 중 1~2개는 영어 원문(+바로 아래 "한국어로 번역" 한 줄)으로 — **매번은 아니되 대략 절반 정도 빈도**로 자연스럽게. 전부 영어로 도배하진 말 것(한국어 댓글이 기본, 영어는 한두 개 양념). (타임스탬프 0:45 형식은 자동으로 파랗게 표시됨.)`, labels: ["댓글","베댓","구독"] },
        ] },
...
[변주 옵션] 비교·대결 구도("어느 쪽이 더 오래갈까?? ← A × B / C × D →"), 참여형 밈("님들아 ~ 해요 우선 나부터"), 인용 캡쳐 받아치기(다른 글 인용 + 짧은 코멘트 "ㄴㄴ 최상"), 자기인용 패러디, 번역 트윗("원문 언어 ○○어"). 감정 폭발 단문도 OK. 닉네임=한글닉+@영문핸들, 인증✓·봇·광고 트윗 섞임. 욕은 텐션용으로 가볍게.
※영어 섞기: 트윗·인용 중 1~2개는 영어(영어 원문 트윗 / 번역 트윗 / 해외 반응 인용 RT 등)로 — **매번은 아니되 대략 절반 정도 빈도**로 자연스럽게. 전부 영어로 하진 말 것(한국어 트윗이 기본, 영어는 한두 개 양념).`, labels: ["실시간","타래","박제"] },
...
function rebuildSubSelect() {
    ...
    if (s.sub === 'namu') s.sub = 'wikipedia';   // 사관백과→위키 결로 병합(v2.39.0)
